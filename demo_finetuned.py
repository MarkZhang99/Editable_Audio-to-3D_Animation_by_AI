...
@torch.no_grad()
def test(args):
    args.feature_dim = getattr(args, "feature_dim", 1024)
    args.bs_dim = getattr(args, "bs_dim", 52)
    args.max_seq_len = getattr(args, "max_seq_len", 512)
    args.period = getattr(args, "period", 20)
    args.emo_gru_hidden = getattr(args, "emo_gru_hidden", 128)
    args.emo_gru_layers = getattr(args, "emo_gru_layers", 2)
    args.transformer_layers = getattr(args, "transformer_layers", 4)
    args.transformer_heads = getattr(args, "transformer_heads", 8)
    args.transformer_dim = getattr(args, "transformer_dim", 512)
    args.num_emotions = getattr(args, "num_emotions", 8)
    args.num_person = getattr(args, "num_person", 25)
    ...
    level = torch.tensor([args.level]).to(args.device)
    person = torch.tensor([args.person]).to(args.device)
...
def render_video(args, frame_count=None):
    wav_name = getattr(args, "wav_name", args.wav_path.split('/')[-1].split('.')[0])
    ...
    # 改写ffmpeg调用
    cmd = f'ffmpeg -r {fps:.2f} -i "{image_temp}" -i "{args.wav_path}" -pix_fmt yuv420p -s 512x768 "{output_path}" -y'
    subprocess.call(cmd, shell=True)

    cmd = f'rm -rf "{image_path}"'
    subprocess.call(cmd, shell=True)
    print(f"✅ 即将生成视频: {output_path}")
...
def main():
    ...
    parser.add_argument("--model_path", type=str, default="./emotalk_finetuned.pth",
    ...
    parser.add_argument("--level", type=int, default=3, help="emotion level label")
    parser.add_argument("--person", type=int, default=0, help="person identity")
    

    args = parser.parse_args()
    prediction = test(args)
    frame_count = prediction.shape[0]  # 注意 shape 是 (T, 52)，你 squeeze 了
    render_video(args, frame_count=frame_count)
    print(f"🧠 输出帧数: {frame_count}, FPS 自动计算中...")
